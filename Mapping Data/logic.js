...
d3.json("./data1.json", createMarkers);
...
function createMarkers(response){
	...
	for(var i=0; i < response.airport.coords.length; i++){
		var airportCoords = [response.airport.coords[i][0], response.airport.coords[i][1]];
		var airportName = response.airport.names[i];
		console.log(airportCoords)
		console.log(airportName)

		var marker = L.circle(airportCoords,{
			color:"black",
			fillColor:"none",
			fillOpacity: 0.5,
			radius: 10000
		...
		marker.addTo(myMap)
		marker.bindPopup("<h3> airport: "+airportName+"</h3>")
	}
}
